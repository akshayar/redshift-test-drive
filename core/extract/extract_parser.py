...
def _parse_user_activity_log(file, logs, databases, start_time, end_time, is_file=True):
...
def _parse_user_activity_log_entry(databases, datetime_pattern, end_time, fetch_pattern, line, logs, start_time, user_activity_log):
    logger.debug("Parsing and validating log entry %s %s %s",start_time, end_time, line)
    if datetime_pattern.match(line):
        if user_activity_log.xid and is_valid_log(
            ...
            logger.debug("Valid log entry %s %s %s %s",filename, start_time, end_time, line)
            if filename in logs:
...
def _parse_start_node_log(file, logs, databases, start_time, end_time, is_file=True):
    start_node_log = Log()
...
def _parse_start_node_log_entry(databases, datetime_pattern, end_time, line, logs, start_node_log, start_time):
    if datetime_pattern.match(line):
